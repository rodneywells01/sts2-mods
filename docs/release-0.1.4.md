# Random Character Options 0.1.4 — review preview

Built by Rodney Wells (WatersEdge) with GPT-6 Astra.

For Slay the Spire 2 **v0.111.0 on Windows**. Prepared as a local review ZIP; this PR does not publish a GitHub release or install into the player's Steam game.
...
Release/core checks, singleplayer lock-in, both reveal modes across three local multiplayer configurations, two native-Random teammate cases, and animation/layout probes passed. See [validation](../packaging/VALIDATION.md) for details and limits. Rodney reported a successful Steam match on the prior version; the new mode still needs that real-session check.
